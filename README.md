...
services:
  stickplayserver:
    image: stickplayserver:latest
    container_name: stickplayserver
    restart: always
    ports:
      - "8099:8099"
    volumes:
...
# 1. 確保已安裝 Docker
# 2. 在根目錄建置 Image
docker build -t stickplayserver:latest .

# 3. 匯出 Image (供 NAS 使用)
docker save stickplayserver:latest -o stickplayserver_image.tar
```
